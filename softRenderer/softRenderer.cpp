...
//D3D matrix perspective fov LH 投影
...
typedef struct {
	...
	texcoord_t tc;
	color_t color;
	float rhw;     //代表某个顶点(x,y,z,w)的齐次点的w的倒数，使用rhw时，告知d3d使用的顶点已经在屏幕坐标系中了，不再执行视图、投影和世界等变换和光线计算。
}	vertex_t;
...
int trapezoid_init_triangle(trapezoid_t *trap, const vertex_t *p1,
	...
	if (p1->pos.y == p2->pos.y) {	// triangle down 平底三角形
	...
	if (p2->pos.y == p3->pos.y) {	// triangle up 平顶三角形
	...
	if (x <= p3->pos.x) {		// triangle left  左边为主
		trap[0].left.v1 = *p1;
		trap[0].left.v2 = *p2;
		trap[0].right.v1 = *p1;
		trap[0].right.v2 = *p3;
...
//画线
void device_draw_line(device_t *device, int x1, int y1, int x2, int y2, IUINT32 color) {
	int x, y, rem = 0;
	if (x1 == x2 && y1 == y2) {
...
int main(void)
{
	...
	const TCHAR *title = _T("Mini3d (software render tutorial) - "); // _T("Left/Right: rotation, Up/Down: forward/backward, Space: switch state");
